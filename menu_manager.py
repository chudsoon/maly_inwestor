...
from exeptions import InsufficientCashError, InsuffincientSharesError
from database import save_wallet, save_transaction_history, load_transaction_history

from utils import generate_transaction_id

class MenuManager:
    def __init__(self, wallet :Wallet, stock_market :dict, transaction_history :dict):
        self.wallet = wallet
    ...
    def handle_buy(self):
        while True:
            try:
                choices = {}
                for k in self.stock_makret.keys():
                    choices[k] = ""
                choices["b"] = ""
                share_ticker  = Prompt.ask("Podaj skrót spółki, którą chcesz kupić lub [bold yellow](b) aby wrócić[/bold yellow]", choices=choices)
                if share_ticker == "b":
                    self.current_state = "main"
                    return
                ...
                self.transaction_history = load_transaction_history()
                self.wallet.refresh_satate(self.transaction_history)
                self.current_state = "main"
                return message
            except InsufficientCashError as e:
                return f"[bold red]Zły stan salda: [/bold red] \n {e}"
    ...
    def handle_sell(self):
        while True:
            try:
                choices = {}
                for k in self.wallet.portfolio.keys():
                    choices[k] = ""
                choices["b"] = ""
                share_ticker  = Prompt.ask("Podaj skrót spółki, którą chcesz sprzedać lub [bold yellow](b) aby wrócić[/bold yellow]", choices=choices)
                if share_ticker == "b":
                    self.current_state = "main"
                    return
